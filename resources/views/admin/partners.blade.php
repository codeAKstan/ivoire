<x-app-layout>
    ...
    @if(session('success'))
        <div class="bg-emerald-50 border-l-4 border-[#18c596] p-6 mb-8 rounded-xl flex items-center gap-4 animate-fadeIn">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-4">
            <div class="bg-white rounded-[30px] p-8 shadow-sm border border-gray-50">
                <div class="flex items-center justify-between mb-8">
                    <h3 class="text-lg font-bold text-[#1b2559]">Current Partners</h3>
                    <span class="text-xs text-gray-400 font-medium tracking-widest uppercase">{{ $partners->count() }} Total</span>
                </div>

                <div class="space-y-6">
                    @forelse($partners as $partner)
                    <div class="flex items-center justify-between p-4 rounded-2xl hover:bg-[#f4f7fe] transition-all group">
                        <div class="flex items-center gap-6">
                            ...
                            <div class="flex flex-col">
                                <span class="text-sm font-semibold text-[#1b2559]">{{ $partner->name }}</span>
                                @if($partner->website)
                                    <a href="{{ $partner->website }}" target="_blank" class="text-[10px] text-[#18c596] hover:underline">{{ $partner->website }}</a>
                                @endif
                            </div>
                        </div>
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-[30px] p-8 shadow-sm border border-gray-50 sticky top-8">
                <h3 class="text-lg font-bold text-[#1b2559] mb-8">Add New Partner</h3>
                ...
                <form action="{{ route('admin.partners.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" placeholder="Enter name..." class="w-full bg-[#f4f7fe] border-none text-[#1b2559] px-6 py-4 rounded-2xl focus:ring-1 focus:ring-[#18c596] transition-all text-sm font-medium placeholder-gray-300">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex items-center gap-4">
                            <label class="flex-1 flex items-center justify-center gap-2 bg-[#18c596] hover:bg-[#15b085] text-white px-6 py-4 rounded-2xl cursor-pointer transition-all shadow-lg shadow-[#18c596]/20 group">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="url" name="website" value="{{ old('website') }}" placeholder="https://..." class="w-full bg-[#f4f7fe] border-none text-[#1b2559] px-6 py-4 rounded-2xl focus:ring-1 focus:ring-[#18c596] transition-all text-sm font-medium placeholder-gray-300">
                        @error('website') <p class="text-red-500 text-[10px] font-bold uppercase mt-1">{{ $message }}</p> @enderror
                    </div>

                    <button type="submit" class="w-full bg-[#1b2559] hover:bg-[#0b1437] text-white font-bold py-5 rounded-2xl uppercase tracking-[0.2em] text-xs transition-all duration-500 shadow-xl shadow-[#0b1437]/20">
                        Register Partner
                    </button>
                </form>
            </div>
