<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto" x-data="{ selectedIcon: '{{ $service->icon }}' }">
        <div class="bg-white rounded-[40px] p-10 shadow-sm border border-gray-50">
            ...
            <form action="{{ route('admin.practices.update', $service) }}" method="POST" class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <!-- Icon Selection -->
                <div class="space-y-4">
                    <label class="text-[10px] font-bold uppercase tracking-widest text-gray-400">Select Icon</label>
                    <input type="hidden" name="icon" x-model="selectedIcon">
                    <div class="grid grid-cols-4 md:grid-cols-6 gap-4">
                        @php
                            $icons = [
                                'transaction', 'family', 'advisory', 'advisory-1', 'advisory-2',
                                'dispute', 'media', 'sports', 'tax', 'technology', 'real-estate', 'company', 'intellectual-property'
                            ];
                            $iconPaths = [
                                ...
                                'advisory' => 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 111.955 0 01-8.618 3.04M12 2.944v18.056',
                    ...
                    @error('icon') <p class="text-red-500 text-[10px] font-bold uppercase mt-1">{{ $message }}</p> @enderror
                </div>
